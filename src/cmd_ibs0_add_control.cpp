...
public:
  ...
  double cm0;
  std::vector<int32_t> carriers;   // IDs of the two carriers
  ibs0interval* ibs0out; // Between non-carrier and carriers

InGroup(int32_t _p, std::string _r, double _l) : pos(_p), oldrec(_r), cm0(_l) {}
~InGroup() {delete ibs0out; ibs0out = NULL;}

void AddID(int32_t _id) {
  carriers.push_back(_id);
}
void Add(double left, double right, double tot) {
  ibs0out = new ibs0interval(left, right, tot);
}
...
// goal -- For each doubleton, add a random sample of an outgroup
int32_t IBS0AddOutgroupControl(int32_t argc, char** argv) {
  ...
  BEGIN_LONG_PARAMS(longParameters)
    ...
    LONG_INT_PARAM("min-hom",&min_hom_gts, "Minimum number of homozygous genotypes to be counted for IBS0")
    LONG_INT_PARAM("min-variant",&min_variant, "Minimum number of variants to present to have output file")
    LONG_INT_PARAM("ibs0-lookup-ck",&ck_len, "Size of ibs0 lookup chunk")
    LONG_DOUBLE_PARAM("max-ibs0",&cm_limit, "Maximal ibs0 in cM to search for.")
    LONG_INT_PARAM("centromere-st",&cst, "Start position of centromere")
    LONG_INT_PARAM("centromere-ed",&ced, "End position of centromere")
  ...
  // Output
  std::ofstream wf;
  outf = out + "_with_outgroup_" + reg + ".txt";
  wf.open(outf, std::ios::trunc);
  ...
  // Setup record of results
  std::vector<InGroup*> Doubts;

  // IBS0 lookup
  notice("Start setting up IBS0 finder");
  IBS0lookup ibs0finder(inVcf, reg, pgmap, cm_limit, ck_len, 1);
  notice("Set up IBS0 finder");
  ...
  while(std::getline(rf, line) && pos < end) {
    ...
    double l0 = std::stod(word);
    line.erase(std::remove(line.begin(), line.end(), '\n'), line.end());
    InGroup* rec = new InGroup(pos, line, l0);
    Doubts.push_back(rec);
  }
  notice("Read input records");
...
//   break;
// }
    // Randomly sample a non-carrier
    int32_t nid = uni(rng);
    while (nid == rec->carriers[0] || nid == rec->carriers[1]) {
      nid = uni(rng);
    }
    // Find no-IBS0 length
    double rcm = 0.0, lcm = 0.0, tcm = 0.0;
    for (int32_t it = 0; it < AC; ++it) {
      int32_t r = ibs0finder.FindIBS0(rec->carriers[it],nid,pos,0);
      int32_t l = ibs0finder.FindIBS0(rec->carriers[it],nid,pos,1);
      if (l < 0) {l = ibs0finder.start_que[0];}
      if (r < 0) {r = ibs0finder.posvec_que.back()->back();}
      if (r-pos <= 0 || pos - l <= 0) {
        notice("Issue: pos %d, left %d, right %d.", pos, l, r);
      }
      rcm += pgmap.bp2cm(r) - pgmap.bp2cm(pos);
      lcm += pgmap.bp2cm(pos) - pgmap.bp2cm(l);
      tcm += pgmap.bp2cm(r) - pgmap.bp2cm(l);
    }
    rcm /= AC; lcm /= AC; tcm /= AC;
    rec->Add(rcm,lcm,tcm);
    // Summarize the control pairs and output for this variant
// std::cout << rec->oldrec << '\t' << rec->ibs0out->leftcm << '\t' << rec->ibs0out->rightcm << '\t' << rec->ibs0out->totcm << '\n';
    wf << rec->oldrec << '\t' << rec->ibs0out->leftcm << '\t' << rec->ibs0out->rightcm << '\t' << rec->ibs0out->totcm << '\n';
    ndone ++;
  }
