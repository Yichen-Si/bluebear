...
public:
  ...
  int32_t fliptype; // Flip vs Blip
  bool flag;        // Priority / Must in.

  Candidate(int32_t id1, int32_t id2, int32_t l, int32_t type, int32_t flip) :
  id(id1), fliptype(flip) {
    supporter.push_back(id2);
    extension.push_back(l);
    flag = flag || type;
  }

  void Update(int32_t id2, int32_t l, int32_t type, int32_t flip) {
    if (flip != fliptype) { // Keep only one type of flip; based on extension
      ...
      if (extension.back() < l) {
        ...
        return; // Ignore the new evidence, keep original
      }
    }
    supporter.push_back(id2);
    extension.push_back(l);
...
int32_t RareOnlyIBS0PhaseBackward(int32_t argc, char** argv) {
  ...
  double  mafcut = 0.05, mafcut_blip = 0.005;
  int32_t rare_ac = 2;
  ...
  BEGIN_LONG_PARAMS(longParameters)
    ...
    LONG_DOUBLE_PARAM("mafcut-blip",&mafcut_blip, "Sample minor allele frequency for considering blipping")
    ...
    LONG_INT_PARAM("max-flip",&max_flip,"Maximal #flips per site if not inside rare variant containing no-ibs0 region")
  ...
  parm << rare_ac << '-' << std::fixed << std::setprecision(1) << (mafcut_blip*100) << '-' << (int32_t) (mafcut*100) << '-' << std::fixed << std::setprecision(1) << delta << '-' << lambda/1000 << '-' << gamma/1000 << '-' << diff/1000 << '-' << nmatch/1000;
  ...
  while (edugly > gpmap.minpos) {
    ...
    for (int32_t k = N-1; k > 0; --k) {
      ...
      for (int32_t it = 0; it < nsamples; ++it) {
        if (flip_abs[it] % 2 == 1 && (gtmat[k][it*2] != gtmat[k][it*2+1]) ) {
      ...
      for (int32_t i = 0; i < M-1; ++i) {
        ...
        while (j < M && dij_p > positions[k] + gamma) {
          h21 = sufpc.a[j];
          h22 = h21 + 1 - 2 * (h21%2);
          if (flipped[h21/2] || h11/2 == h21/2) { // Probably long homozygous sites
            j++;
            if (sufpc.d[j] < dij_p)
              dij_p = sufpc.d[j];
            continue;
          ...
          // If a match ends at k-1
            ...
            if (theshort - thelong > diff && positions[k] - thelong > nmatch) {
              fliptype = 1;
            } else if (mafs[k-1] < mafcut_blip  && (gtmat[k-1][h12] == gtmat[k-1][h21] || gtmat[k-1][h11] == gtmat[k-1][h22]) && k > 1) { // Consider blip
              i_s = rmat[k-2][h11]; j_s = rmat[k-2][h21];
              int32_t lower = std::min(i_s, j_s);
              int32_t upper = std::max(i_s, j_s);
              thelong = dmat[k-2][lower + 1];
              for (int32_t it = lower + 1; it <= upper; ++it) { // Evaluate d_i'j+
                if (dmat[k-2][it] > thelong)
                  thelong = dmat[k-2][it];
              }
              theshort = positions[k];
              // Decide which person to blip
              bool minor = 1;
              if (oppo[k-1]) { // 0 labels the minor allele
                minor = 0;
              }
              if (gtmat[k-1][h11] == minor) {
                if (gtmat[k-1][h22] == minor) {thewho = h21/2; theother = h11/2;}
                else {thewho = h11/2; theother = h21/2;}
              }
              if (gtmat[k-1][h21] == minor) {
                if (gtmat[k-1][h12] == minor) {thewho = h11/2; theother = h21/2;}
                else {thewho = h21/2; theother = h11/2;}
              }
              if (positions[k] - thelong > nmatch) {
                fliptype = 2;
              }
            }
            if (fliptype > 0) {
              ...
              int32_t nextibs0 = 0;
              int32_t rarepos = nextrare; // Pos of closest shared rare variant
              if (nextrare - positions[k] < positions[k] - prevrare) {
                rarepos = prevrare;
              }
              ...
              if (flag) { // Flip
                fliprec.push_back(std::vector<int32_t> {positions[k-1],thewho,theother,
                                                        dij_p, theshort, thelong, flag, fliptype, j-i, rarepos});
                if (flipcandy.find(thewho) != flipcandy.end()) {
                  flipcandy[thewho]->Update(theother, positions[k]-thelong, (flag==2), fliptype);
                } else {
                  Candidate* candy = new Candidate(thewho,theother,positions[k]-thelong,(flag == 2), fliptype);
      ...
      if (flipcandy.size() > 0) {
        ...
        if (flipcandy.size() > 1) {
          ...
          for (auto & v : toflip) {
            ...
            if (rm) {
              flipped[v->id] = 1;
              finalrec[v->id] = std::vector<int32_t>{0,v->id,0,0, 0,0,0, v->fliptype, 0};
            }
          }
        } else {
          flipped[flipcandy.begin()->first] = 1;
          finalrec[flipcandy.begin()->first] = std::vector<int32_t>{0,flipcandy.begin()->first,0,0, 0,0,0, (flipcandy.begin()->second)->fliptype, 0};
        }
        for (auto & v : fliprec) {
          if (flipped[v[1]]) {
            finalrec[v[1]][0] = v[0];
            finalrec[v[1]][2] = int32_t (mafs[k-1] * M);
            ...
            finalrec[v[1]][4] = v[9]; // Rare allele position
            if (finalrec[v[1]][8] > v[8] || finalrec[v[1]][8] == 0)
              finalrec[v[1]][8] = v[8]; // Row dist. in pbwt
            if (detailed) {
