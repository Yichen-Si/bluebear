...
int32_t RareBlipOnlyForward(int32_t argc, char** argv) {
  ...
  int32_t ibs0_chunk = 1000000;          // Store ibs0 look up blocks by chunk
  int32_t start_pos = 0; // Starting position

  int32_t rare_ac = 0;
  int32_t diff = 5000, safediff = -1;  // Difference of hap match on each side
  double  delta = 1.0;  // Minimal no-ibs0 length with any other carriers
  double  mafcut = 0.0;
  bool bothside = 0;
  ...
  BEGIN_LONG_PARAMS(longParameters)
    ...
    LONG_DOUBLE_PARAM("no-ibs0",&delta, "Minimal no-ibs0 length with any other carriers")
    LONG_INT_PARAM("rare-ac",&rare_ac,"Rare allele sharing as evidence for IBD")
    LONG_INT_PARAM("min-diff",&diff,"The minimal hap extension (in bp) for a blip to be recorded")
    LONG_INT_PARAM("safe-diff",&safediff,"The minimal hap extension (in bp) on each side for a blip to be recorded regardless of no-ibs0 length")
    LONG_INT_PARAM("start-from",&start_pos,"Starting position. Proceed on both sides.")
    LONG_INT_PARAM("both-side",&bothside,"If the criterion is to be satisfied by both sides.")
  ...
  parm << std::fixed << std::setprecision(1) << mafcut*1000 << '-' << diff/1000 << '-' << safediff/1000 << '-' << std::fixed << std::setprecision(1) << delta << '-' << bothside;
  ...
  while (st < pgmap.maxpos) {
    ...
    for (int32_t k = 0; k < N-2; ++k) {
      ...
      int32_t max_pref_id = 0, max_suff_id = 0;
...
// std::cout << max_dpref << '\t' << max_dsuff << '\t' << positions[k+1] << "\n";
      ...
      // Conditional on a certain no-ibs0 length to reduce false positive.
      int32_t left[2], right[2];
      ...
      for (int32_t it = 0; it < m; ++it) {
        for (int32_t jt = 0; jt < 2; ++jt) {
          ...
          left[jt] = positions[k+1] -left[jt];
          right[jt] = right[jt] - positions[k+1];
// std::cout << left[jt] << '\t' << right[jt] << "\n";
        ...
        if (gtmat[k+1][carriers[it]*2+lindx] == 1) {continue;}
        // int32_t previbs0 = ibs0finder.FindIBS0(carriers[it],max_pref_id,positions[k+1],1);
        // int32_t nextibs0 = ibs0finder.FindIBS0(carriers[it],max_suff_id,positions[k+1],0);
        bool bliphere = 0;
        if ( safediff > 0 && left[lindx] > safediff && right[lindx] > safediff) {
          // Don't need to check ibs0
          bliphere = 1;
        } else if ( (bothside && (left[lindx] > diff && right[lindx] > diff)) ||
                   (!bothside && left[lindx] + right[lindx] > diff) ) {
          int32_t previbs0 = ibs0finder.FindIBS0(carriers[it],max_pref_id,positions[k+1],1);
          int32_t nextibs0 = ibs0finder.FindIBS0(carriers[it],max_suff_id,positions[k+1],0);
          if (nextibs0 < 0 || previbs0 < 0) {
            bliphere = 1;
          } else if (pgmap.bp2cm(nextibs0) - pgmap.bp2cm(previbs0) > delta) {
            bliphere = 1;
          }
        }
        if (bliphere) {
          ...
          recline << positions[k+1] << '\t' << m << '\t' << carriers[it] << '\t' << left[1-lindx] << '\t' << right[1-lindx] << '\t' << left[lindx] << '\t' << right[lindx] << '\n';
          // if (previbs0 > 0) {
          //   recline << 1 << '\t' << positions[k+1] - previbs0 << '\t';
          // } else {
          //   recline << 0 << '\t' << positions[k+1] - ibs0finder.start_que[0] << '\t';
          // }
          // if (nextibs0 > 0) {
          //   recline << 1 << '\t' << nextibs0 - positions[k+1] << '\n';
          // } else {
          //   recline << 0 << '\t' << ibs0finder.posvec_que.back()->back() - positions[k+1] << '\n';
          // }
          wf << recline.str();
...
int32_t RareBlipOnlyStats(int32_t argc, char** argv) {
  ...
  std::ostringstream parm;
  parm << std::fixed << std::setprecision(1) << mafcut*1000 << '-' << std::fixed << std::setprecision(1) << delta;
  notice("Parameter setting: %s.", parm.str().c_str());
  ...
  while (st < pgmap.maxpos) {
    ...
    reg = chrom + ":" + std::to_string(st) + "-" + std::to_string(ed);
    std::ofstream wf;
    outf = out + "_" + parm.str() + "_st_" + std::to_string(start_pos) + "_fw_" + reg + ".blip.stats";
    wf.open(outf, std::ios::trunc);
    ...
    for (int32_t k = 0; k < N-2; ++k) {
      ...
      // Find rare allele carriers
      std::vector<int32_t> carriers;
      std::vector<int32_t> carryhap;
      std::vector<int32_t> prefindx;
      std::vector<int32_t> suffindx;
      for (int32_t i = 0; i < nsamples; ++i) {
        if (gtmat[k+1][2*i]==minor) {
          carryhap.push_back(0);
          carriers.push_back(i);
        }
        if (gtmat[k+1][2*i+1] == minor) {
          carryhap.push_back(1);
          carriers.push_back(i);
        }
      }
      int32_t m = (int32_t) carriers.size();
...
// std::cout << left[jt] << '\t' << right[jt] << "\n";
        ...
        int32_t previbs0 = ibs0finder.FindIBS0(carriers[it],max_pref_id,positions[k+1],1);
        int32_t nextibs0 = ibs0finder.FindIBS0(carriers[it],max_suff_id,positions[k+1],0);
        lindx = (left[0] > left[1]) ? 0 : 1;
        rindx = (right[0] > right[1]) ? 0 : 1;
        std::stringstream recline;
        recline << positions[k+1] << '\t' << m << '\t' << carriers[it] << '\t' << left[carryhap[it]] << '\t' << right[carryhap[it]] << '\t' << left[lindx] << '\t' << right[rindx] << '\t';
        if (previbs0 > 0) {
          recline << 1 << '\t' << positions[k+1] - previbs0 << '\t';
        } else {
          recline << 0 << '\t' << positions[k+1] - ibs0finder.start_que[0] << '\t';
