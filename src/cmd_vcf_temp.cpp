...
//     bool haploid = false, an_doubled = false;
//     int32_t debug = 0;
//     bool fold = false;
...
//         LONG_INT_PARAM("min-ac",&min_ac,"Minimum allele count to consider")
...
//     int32_t  nVariant = 0, nUniq = 0, nPASS = 0, nFAIL = 0, nMiss = 0, nMissSkip = 0;
...
//         int32_t nmiss = 0;
//         int32_t ret = 0;
// if (debug % 3 == 1 || (debug && nVariant % (debug/10) == 0)) {
// std::cout << iv->pos+1 << " " << info_ac[0] << " " << nUniq << "/" << nVariant << " (" << nPASS << ',' << nFAIL << ')' << '\t' << hmtx.nValues << std::endl << std::flush;
// }
//         if (sparse) {
...
//                     if ( bcf_gt_is_missing(p_gt[i*2]) ) {
//                         nmiss++;
//                         gt[i] = flip ? 1 : 0; // so not to affect hashing
//                     } else {
...
//             }
//             if (ac1 != info_ac[0]) {
//                 notice("AC mismatch: %d vs %d at %d (%d), missing %d", info_ac[0], ac1, iv->pos+1, nVariant, nmiss);
//             }
//             if (nmiss > max_missing_rate * nalleles) {
//                 nMissSkip++;
//                 continue;
//             }
// if (debug % 3 == 1 || (debug && nVariant % (debug/10) == 0)) {
//     std::cout << iv->pos+1 << " add_in" << std::endl << std::flush;
// }
//             ret = hmtx.add_int<uint8_t>(gt, ac1);
//         }
// if (debug % 3 == 1 || (debug && nVariant % (debug/10) == 0)) {
//     std::cout << iv->pos+1 << " " << ret << std::endl << std::flush;
// }
//         if (ret < 0) {
//             ++nUniq;
...
//     bcf_destroy(iv);
//     odr.close();
//     notice("Finished. Processed %d variants, identified %d unique ones. Query (hash) %d times, need to do full comparison %d times, %d times found exact match", nVariant, nUniq, hmtx.nQuery, hmtx.nCompare, hmtx.nCollision);
