...
struct gap {
  int32_t pos, length, pref, suff, jump;
  gap( int32_t _p, int32_t _l, int32_t _f, int32_t _s ) : pos(_p), length(_l), pref(_f), suff(_s) {
    jump = 1;
    if (_f == _s || _f < 0 || _s < 0) {
      jump = 0;
    }
  }
  bool operator < (const gap & v) const
  {
    if (length < v.length) {
      return (true);
    }
...
int32_t phase_match_order(int32_t* indx, int32_t* gt, int32_t N, int32_t* res) {
  int32_t gv[N];
  for (int32_t i = 0; i < N; ++i) {
    gv[i] = gt[i*2] + gt[i*2+1];
  }
  if (res == NULL) {
    int32_t* pt[N*2];
    res = pt;
