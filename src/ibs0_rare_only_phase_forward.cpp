        ...
        while (j < M && dij_p < positions[k] - gamma) {
          ...
          // If a match ends at k+1
            ...
            if (thelong - theshort > diff && thelong - positions[k] > nmatch) {
              fliptype = 1;
            } else if (mafs[k+1] < mafcut_blip && (gtmat[k+1][h12] == gtmat[k+1][h21] || gtmat[k+1][h11] == gtmat[k+1][h22]) && k+2 < N) { // Consider blip
            ...
            if (fliptype > 0) {
              ...
              if (flag) { // Flip
                ...
                if (flipcandy.find(thewho) != flipcandy.end()) {
                  flipcandy[thewho]->Update(theother, thelong-positions[k], (flag==2), fliptype);
                } else {
                  Candidate* candy = new Candidate(thewho,theother,thelong-positions[k],(flag == 2),fliptype);
