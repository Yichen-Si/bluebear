...
int32_t cmdVcfIBS0Baseline(int32_t argc, char** argv) {
  ...
  std::vector<int32_t> bincount(nbins+1, 0);
  ...
  for(int32_t i=1; i < nsamples; ++i) {
    ...
    for(int32_t j=0; j < i; ++j) {
        ...
        for (int32_t bit=0; bit<8 && k*8+bit<bmatRR.ncol; ++bit) {
          ...
          if (mac[pt] <= max_rare_ac) { // rare variant
            if ( ((iAA[k]>>(7-bit)) & 0x01) & ((jAA[k]>>(7-bit)) & 0x01) ) { // share rare allele
              if (nibs0 > 2 && rec.size() > 0)
                rec.pop_back(); // Remove the imediate previous no-ibs0 region
              nibs0 = 0; // Reset ibs0 counter
            }
            else {continue;}
          }
        	else if ( (byte >> (7-bit)) & 0x01 ) { // IBS0
            if ( nibs0 > 1 )
              rec.push_back(snoopy[pt]-prepos);
            prepos = snoopy[pt];
        	  nibs0++;
        	}
        }
      ...
      for (uint32_t it = 0; it < rec.size(); it++) {
        int32_t pt = rec[it]/bin_size/1000;
