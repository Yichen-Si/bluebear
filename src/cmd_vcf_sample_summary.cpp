...
int32_t cmdVcfSampleSummary(int32_t argc, char** argv) {
  ...
  BEGIN_LONG_PARAMS(longParameters)
    ...
    LONG_PARAM("haploid",&haploid,"VCF contains haploid genotypes")
  ...
  for(int32_t k=0; odr.read(iv); ++k) {  // read marker
    ...
    if ( ( !reg.empty() ) && posOnly &&
         ( ( intervals[0].start1 > iv->pos+1 ) ||
           ( intervals[0].end1 < iv->pos+1 ) ) ) {
    ...
    if ( countVariants ) {
        ...
        for(int32_t i=0, j=12; i < (int32_t)acThres.size(); ++i, j += 2) {
            if ( ( non_ref_ac > (i == 0 ? 0 : acThres[i-1]) ) &&
                 ( non_ref_ac <= acThres[i] ) ) {
                if ( bcf_is_snp(iv) ) {
        ...
        for(int32_t i=0, j=12 + 2*acThres.size(); i < (int32_t)afThres.size(); ++i, j += 2) {
            if ( ( non_ref_ac > (i == 0 ? 0 : afThres[i-1]*an) ) &&
                 ( non_ref_ac <= afThres[i]*an ) ) {
                if ( bcf_is_snp(iv) ) {
                    varMasks[j] = MASK_GT_NONREF;
                }
                else {
        ...
        if (haploid) {
            for(int32_t i=0; i < nsamples; ++i) {
                int32_t g1 = p_gt[i];
                int32_t geno; // 0: missing 1: ref, 3: alt
                if ( bcf_gt_is_missing(g1) ) {
                    geno = 0;
                }
                else {
                    geno = (bcf_gt_allele(g1) > 0) ? 3 : 1;
                }
                p_gt[i] = geno;
            }
        } else {
            for(int32_t i=0; i < nsamples; ++i) {
                ...
                int32_t geno; // 0: missing (any of two alleles) 1: hom-ref, 2: het, 3: hom-alt
...
// Goal: output per-sample singleton count by window
int32_t cmdVcfCountSingleton(int32_t argc, char** argv) {
  std::string inVcf;
  std::string out;
  std::string reg, chrom;
  int32_t verbose = 1000;
  int32_t wsize   = 10000;
  ...
  BEGIN_LONG_PARAMS(longParameters)
    ...
    LONG_STRING_PARAM("out", &out, "Output VCF file name")
    LONG_INT_PARAM("verbose",&verbose,"Frequency of verbose output (1/n)")
    LONG_INT_PARAM("window-size",&wsize,"Window size in bp")
  END_LONG_PARAMS();
  ...
  // Output files
  std::string outf = out + ".persample.singleton.w" + std::to_string(wsize/1000) + "kb.ct";
  std::ofstream pwf(outf.c_str());
  outf = out + ".singleton.snp";
  std::ofstream cwf(outf.c_str());
  ...
  if ( !reg.empty() ) {
    ...
    split(w, ":-", reg);
    chrom = w[0];
    if (!str2int32(w[1], start)) {
      error("Invalid region.");
    }
  }
  start = (start / wsize) * wsize;
  ...
  int32_t nVariant = 0;
  int32_t nsamples = bcf_hdr_nsamples(odr.hdr);
  char** ptr = bcf_hdr_get_samples(odr.hdr);
  std::vector<std::string> id_samples(ptr, ptr+nsamples);
  std::vector<int32_t> ct(nsamples, 0);

  pwf << "CHROM\tStart\tEnd";
  for (const auto & v : id_samples)
    pwf << '\t' << v;
  pwf << '\n';
  ...
  int32_t  pre_win = start;
  int32_t  pos = 0;

  for(int32_t k=0; odr.read(iv); ++k) {  // read marker

    pos = iv->pos+1;
    if (pos >= pre_win + wsize) {
      pwf << chrom << '\t' << pre_win << '\t' << pre_win + wsize - 1;
      for (int32_t i = 0; i < nsamples; ++i) {
        pwf << '\t' << ct[i];
        ct[i] = 0;
      }
      pwf << '\n';
      pre_win += wsize;
      while(pos >= pre_win + wsize) {
        pre_win += wsize;
      }
    }

    if (!bcf_is_snp(iv)) {continue;}
    if ( (nVariant+1) % verbose == 0 )
      notice("Processing %d informative variants at %s:%d.", nVariant+1, bcf_hdr_id2name(odr.hdr, iv->rid), iv->pos+1);
    ...
    if (info_ac[0] != 1 && info_ac[0] != nsamples-1 ) {continue;}
    int32_t minor = 1;
    if (info_ac[0] == nsamples-1) {minor=0;}
    ...
    for(int32_t i=0; i < nsamples; ++i) {
      ...
      int32_t g2 = p_gt[2*i+1];
      if ( !bcf_gt_is_missing(g1) && !bcf_gt_is_missing(g2) ) {
        if ( ((bcf_gt_allele(g1) > 0) ? 1 : 0) + ((bcf_gt_allele(g2) > 0) ? 1 : 0) == minor ) {
          ct[i]++;
          indx = i;
          break;
        }
